...
import { WeekSchedule } from 'src/app/models/WeekSchedule.model';
...
export class StudentBookService {
  ...
  public getFileById(lessonId: number): Observable<File> {
    return this._http.get<any>(`/homeworks/files/${lessonId}`).pipe(map(response => {
  ...
  public getDiariesList(date: Date = new Date()): Observable<WeekSchedule[]> {
    ...
    return this._http.get<any>(`/diaries?weekStartDate=${formattedDate}`).pipe(
      map(response => {
